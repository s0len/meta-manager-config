#!/usr/bin/env python3
"""Generate NBA metadata YAML using TheSportsDB rounds feed.

This script mirrors :mod:`generate_ufc_metadata` but targets the NBA
(``league_id 4387``). It groups SportsDB fixtures by NBA week, emits a season
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate NBA metadata YAML using TheSportsDB rounds feed.",
    )
    parser.add_argument(
        "--season",
        default="2025-2026",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4387,
        help="TheSportsDB league/competition ID for the NBA.",
    )
    parser.add_argument(
        "--api-key",
        default="123",
    ...
    parser.add_argument(
        "--poster-url",
        default="posters/nba/{season}/poster.jpg",
        help="Show-level poster path or URL (supports {season}).",
    )
    parser.add_argument(
        "--background-url",
        default="posters/nba/{season}/background.jpg",
        help="Show-level background path or URL (supports {season}).",
    )
    parser.add_argument(
        "--summary",
        default=(
            "The {season} NBA season features weekly national and international "
            "matchups across the Association. Each SportsDB round is grouped "
            "here so NBA week recordings can be matched and renamed "
            "automatically."
        ),
    ...
    parser.add_argument(
        ...
        default="posters/nba/{season}/s{matchweek_token}/poster.jpg",
        help=(
            "Relative path template for NBA week (season-level) posters. "
    ...
    parser.add_argument(
        ...
        default="posters/nba/{season}/s{matchweek_token}/e{episode_index}.jpg",
        help=(
    ...
    parser.add_argument(
        ...
        "(defaults to metadata-files/nba/{season}.yaml).",
    )
    parser.add_argument(
        "--insecure",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="First NBA week number to include (default: 1).",
    )
    parser.add_argument(
        "--matchweek-stop",
        type=int,
        default=DEFAULT_MATCHWEEK_STOP,
        help=f"Last NBA week number to include (default: {DEFAULT_MATCHWEEK_STOP}).",
    )
    parser.add_argument(
        "--matchweek-delay",
        type=float,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if output_path is None:
        ...
        output_path = Path("metadata-files") / f"nba/{safe_season}.yaml"
    output_path = output_path.expanduser()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(yaml_text, encoding="utf-8")
    print(f"Wrote NBA metadata to {output_path}")
    return 0
