#!/usr/bin/env python3
"""Generate FIS Alpine Ski World Cup metadata via TheSportsDB.

This helper mirrors :mod:`generate_premier_league_metadata_sportsdb` but targets
the Alpine Ski World Cup feed (``league_id 5625``). Every SportsDB round becomes
an Alpine "stop" season entry and each event listed under that round is emitted
as its own episode, complete with venue/date summaries and poster URLs. The CLI
surface matches the UFC/Premier League tooling so we inherit the shared throttled
downloader, exponential backoff and SSL bypass path.
"""
...
def fetch_round_events(
    ...
    if round_url:
        return events
    return [event for event in events if _round_number_from_event(event) == round_number]
...
def _round_number_from_event(event: dict) -> Optional[int]:
    for value in (event.get("intRound"), event.get("strRound")):
        if value in (None, "", "0"):
            continue
        try:
            return int(value)
...
def _fixture_sort_key(event: dict) -> Tuple[date, str]:
    event_date = _date_from_event(event)
    name = _event_display_name(event).lower()
    return (event_date, name)


def _pick_poster_source(event: dict) -> Optional[str]:
    for field in ("strPoster", "strEventPoster", "strThumb"):
        url = event.get(field)
        if url:
            return url
    return None
...
def _pick_background_source(event: dict) -> Optional[str]:
    for field in ("strFanart", "strFanart1", "strFanart2"):
        url = event.get(field)
        if url:
            return url
    return None
...
def _pick_episode_thumb(event: dict) -> Optional[str]:
    for field in ("strThumb", "strEventThumb", "strPoster", "strBanner"):
        url = event.get(field)
        if url:
            return url
    return None
...
def _build_stop_title(stop_label: str, stop_number: int, representative: Optional[dict]) -> str:
    base = f"{stop_label} {stop_number}"
    if not representative:
        return base
    location = _event_location(representative)
    if location:
        return f"{base}: {location}"
    event_name = _event_display_name(representative)
    if event_name:
        return f"{base}: {event_name}"
    return base
...
def build_metadata(args: argparse.Namespace, sportsdb: SportsDBSettings) -> dict:
            ...
            try:
                ...
                if verify_ssl:
                    print(
                        ...
                        f"for {args.stop_label.lower()} {round_number}...",
                        file=sys.stderr,
                    )
                    context = build_ssl_context(False)
                    round_events = fetch_round_events(
    ...
    for round_number in sorted(events_by_round):
        ...
        season_poster_url = ""
        stop_slug = slugify(f"{args.stop_label}-{round_number}")
        representative_event = fixtures[0] if fixtures else None
        ...
        for index, event in enumerate(fixtures, start=1):
            ...
            if args.fixture_poster_template:
                ...
                discipline_slug = slugify(event.get("strEventAlternate") or event.get("strDescriptionEN") or event_name)
            ...
            episodes.append(
                {
                    ...
                    "summary": _build_episode_summary(event, round_number, args.stop_label),
        ...
        if match_count and date_span:
            summary = (
                f"{args.stop_label} {round_number} spans {date_span} with "
                f"{match_count} runs published in TheSportsDB feed."
            )
        elif match_count:
            summary = (
                f"{args.stop_label} {round_number} currently lists "
                f"{match_count} runs in TheSportsDB feed."
            )
        else:
            summary = (
                f"{args.stop_label} {round_number} currently has no runs available "
                "from TheSportsDB."
            )
        host_location = _event_location(representative_event)
        if host_location:
            summary += f" Host venue: {host_location}."

        seasons.append(
            {
                "number": round_number,
                "title": _build_stop_title(args.stop_label, round_number, representative_event),
                "sort_title": f"{round_number:02d}_{args.stop_label} {round_number}",
                "summary": summary,
...
def render_yaml(metadata: dict) -> str:
    ...
    lines.append(f"    url_poster: {metadata.get('poster_url', '')}")
    ...
    for season in metadata["seasons"]:
        ...
        lines.append(f"        url_poster: {season.get('url_poster', '')}")
        lines.append("        summary: >")
        ...
        for episode in season["episodes"]:
            ...
            lines.append(f"            url_poster: {episode.get('url_poster', '')}")
            lines.append("            summary: >")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate FIS Alpine Ski World Cup metadata via TheSportsDB.",
    )
    parser.add_argument(
        "--season",
        default=DEFAULT_SEASON,
        help="Season identifier passed to TheSportsDB (e.g. 2026).",
    )
    parser.add_argument(
        "--league-id",
        type=int,
        default=5625,
        help="TheSportsDB league/competition ID for the Alpine Ski World Cup.",
    )
    parser.add_argument(
        "--api-key",
        default=SPORTSDB_DEFAULTS.api_key,
        help="TheSportsDB API key (defaults to SPORTSDB_API_KEY or 123).",
    )
    parser.add_argument(
        "--api-version",
        default=SPORTSDB_DEFAULTS.api_version,
        help="TheSportsDB API version to target (v1 or v2).",
    )
    parser.add_argument(
        "--title",
        default="FIS Alpine Ski World Cup {season}",
        help="Show title used in the YAML (supports {season}).",
    )
    parser.add_argument(
        "--sort-title",
        default=None,
    ...
    parser.add_argument(
        "--show-id",
        default="FIS Alpine Ski World Cup {season}",
        help="Metadata key used under the top-level 'metadata:' block (supports {season}).",
    )
    parser.add_argument(
        "--poster-url",
        default="posters/fis-alpine-world-cup/{season}/poster.jpg",
        help="Show-level poster path or URL (supports {season}).",
    )
    parser.add_argument(
        "--background-url",
        default="posters/fis-alpine-world-cup/{season}/background.jpg",
        help="Show-level background path or URL (supports {season}).",
    )
    parser.add_argument(
        "--summary",
        default=(
            "The {season} FIS Alpine Ski World Cup calendar spans glacier openers "
            "through the season finale. Each stop groups the races listed by "
            "TheSportsDB so recordings can be matched and renamed automatically."
        ),
        help="Overall show summary text (supports {season}).",
    )
    parser.add_argument(
        "--stop-label",
        default=DEFAULT_STOP_LABEL,
        help="Label used for each season entry (defaults to 'Stop').",
    )
    parser.add_argument(
        "--asset-url-base",
    ...
    parser.add_argument(
        ...
        default="posters/fis-alpine-world-cup/{season}/s{matchweek}/poster.jpg",
        help=(
            "Relative path template for stop posters. Supports "
            "{season}, {matchweek}, {matchweek_token}, {stop}, {stop_label}."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="posters/fis-alpine-world-cup/{season}/s{matchweek}/e{episode_index}.jpg",
        help=(
            "Relative path template for per-race posters. Supports "
            "{season}, {matchweek}, {matchweek_token}, {stop}, {event_slug}, "
            "{discipline_slug}, {episode_index}."
        ),
    )
    parser.add_argument(
        "--skip-asset-download",
    ...
    parser.add_argument(
        ...
        "(defaults to metadata/fis-alpine-world-cup/{season}.yaml).",
    )
    parser.add_argument(
        "--insecure",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="First stop number to include (default: 1).",
    )
    parser.add_argument(
        "--matchweek-stop",
        type=int,
        default=DEFAULT_MATCHWEEK_STOP,
        help="Last stop number to include (default: 25).",
    )
    parser.add_argument(
        "--matchweek-delay",
        type=float,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    def resolve_asset(value: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
        ...
        asset_url = build_asset_url(args.asset_url_base, formatted)
    ...
    tokens = {"season": args.season, "stop_label": args.stop_label}
    if args.summary:
        args.summary = args.summary.format(**tokens)
    if args.title:
        args.title = args.title.format(**tokens)
    if args.sort_title:
        args.sort_title = args.sort_title.format(**tokens)
    if args.show_id:
        args.show_id = args.show_id.format(**tokens)
    ...
    if output_path is None:
        ...
        output_path = Path("metadata") / f"fis-alpine-world-cup/{safe_season}.yaml"
    output_path = output_path.expanduser()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(yaml_text, encoding="utf-8")
    print(f"Wrote FIS Alpine Ski World Cup metadata to {output_path}")
    return 0
