#!/usr/bin/env python3
"""Generate Formula 3 metadata YAML using TheSportsDB rounds feed.

This script mirrors :mod:`generate_ufc_metadata` but targets Formula 3
(``league_id 4487``). It groups SportsDB fixtures by round, emits a season entry
per race weekend, and writes an episode for every session returned by the feed
without injecting any extra placeholders.
"""
...
FORMULA_TAG_RE = re.compile(r"\b(?:fia\s+)?formula\s*3\b[:\s-]*", re.IGNORECASE)
FORMULA_EVENT_PREFIX_RE = re.compile(
    r"^\s*(?:fia\s+)?formula\s*3\s*[-:–—]*\s*", re.IGNORECASE
)
...
def _season_summary(
    ...
    return (
        f"{round_title} ({round_label} {round_number}) currently has no Formula 3 "
        "entries available from TheSportsDB."
    )


def _episode_title(event: dict, round_title: str, index: int) -> str:
    for field in ("strEvent", "strEventAlternate", "strFilename"):
        raw = event.get(field)
        if not raw:
            continue
        cleaned = FORMULA_EVENT_PREFIX_RE.sub("", raw).strip()
        cleaned = re.sub(r"\s+", " ", cleaned)
        if cleaned:
            return cleaned
    return f"{round_title} Session {index}"
...
def build_metadata(args: argparse.Namespace, sportsdb: SportsDBSettings) -> dict:
    ...
    for round_number in sorted(events_by_round):
        ...
        for index, event in enumerate(fixtures, start=1):
            event_title = _episode_title(event, round_title, index)
            ...
            if args.fixture_poster_template:
                poster_source_url = _pick_episode_thumb(event)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Formula 3 metadata YAML using TheSportsDB rounds feed.",
    )
    parser.add_argument(
        "--season",
        default="2025",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4487,
        help="TheSportsDB league/competition ID for Formula 3.",
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        "--poster-url",
        default="posters/formula3/{season}/poster.jpg",
        help="Show-level poster path or URL (supports {season}).",
    )
    parser.add_argument(
        "--background-url",
        default="posters/formula3/{season}/background.jpg",
        help="Show-level background path or URL (supports {season}).",
    )
    parser.add_argument(
        "--summary",
        default=(
            "The {season} FIA Formula 3 Championship shares the Formula 1 calendar, "
            "delivering sprint and feature races at every Grand Prix weekend. Each "
            "SportsDB round is grouped here so race-weekend recordings can be renamed "
    ...
    parser.add_argument(
        ...
        default="posters/formula3/{season}/s{matchweek_token}/poster.jpg",
        help=(
    ...
    parser.add_argument(
        ...
        default="posters/formula3/{season}/s{matchweek_token}/e{episode_index}.jpg",
        help=(
    ...
    parser.add_argument(
        ...
        "(defaults to metadata/formula3/{season}.yaml).",
    )
    parser.add_argument(
        "--insecure",
        action="store_true",
    ...
    parser.add_argument(
        "--round-label",
        default="Round",
        help="Label used when describing each Formula 3 round (e.g. Round, Grand Prix).",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if output_path is None:
        ...
        output_path = Path("metadata") / f"formula3/{safe_season}.yaml"
    output_path = output_path.expanduser()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(yaml_text, encoding="utf-8")
    print(f"Wrote Formula 3 metadata to {output_path}")
    return 0
