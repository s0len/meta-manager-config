#!/usr/bin/env python3
"""Generate NHL metadata YAML using TheSportsDB rounds feed.

This script mirrors :mod:`generate_ufc_metadata` but targets the NHL
(``league_id 4380``). It groups SportsDB fixtures by NHL week, emits a season
entry per week, and includes an episode for every scheduled game.
"""
...
def _build_fixture_summary(event: dict, matchweek: int) -> str:
    ...
    event_date = _date_from_event(event)
    summary_parts = [
        f"NHL Week {matchweek} game at {venue}"
...
def build_metadata(args: argparse.Namespace, sportsdb: SportsDBSettings) -> dict:
        ...
        for round_number in range(args.matchweek_start, args.matchweek_stop + 1):
            ...
            try:
                ...
                if verify_ssl:
                    ...
                    raise RuntimeError(
                        f"Failed to fetch NHL week {round_number}: {exc}"
                    ) from exc
    ...
    for round_number in sorted(events_by_round):
        ...
        if args.matchweek_poster_template:
            ...
            # Season posters are curated manually; skip auto-downloading from fixtures.
        ...
        if match_count and date_span:
            summary = (
                f"NHL Week {round_number} spans {date_span} with "
                f"{match_count} games recorded in TheSportsDB feed for "
                f"the {args.season} NHL regular season."
            )
        elif match_count:
            summary = (
                f"NHL Week {round_number} currently lists {match_count} "
                "games in TheSportsDB feed."
            )
        else:
            summary = (
                f"NHL Week {round_number} currently has no games "
        ...
        seasons.append(
            {
                "number": round_number,
                "title": f"NHL Week {round_number}",
                "sort_title": f"{round_number:02d}_NHL Week {round_number}",
                "summary": summary,
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate NHL metadata YAML using TheSportsDB.",
    )
    parser.add_argument(
        "--season",
        default="2025-2026",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4380,
        help="TheSportsDB league/competition ID for the NHL.",
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        "--poster-url",
        default="posters/nhl/{season}/poster.jpg",
        help="Show-level poster path or URL (supports {season}).",
    )
    parser.add_argument(
        "--background-url",
        default="posters/nhl/{season}/background.jpg",
        help="Show-level background path or URL (supports {season}).",
    )
    parser.add_argument(
        "--summary",
        default=(
            "The {season} NHL regular season spans North America across weekly "
            "slates. Each SportsDB round maps to an NHL week so recordings can be "
            "grouped, tagged and renamed automatically."
        ),
    ...
    parser.add_argument(
        ...
        default="posters/nhl/{season}/s{matchweek}/poster.jpg",
        help=(
            "Relative path template for NHL week (season-level) posters. "
    ...
    parser.add_argument(
        ...
        default="posters/nhl/{season}/s{matchweek}/e{episode_index}.jpg",
        help=(
    ...
    parser.add_argument(
        ...
        "(defaults to metadata/nhl/{season}.yaml).",
    )
    parser.add_argument(
        "--insecure",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="First NHL week number to include (default: 1).",
    )
    parser.add_argument(
        "--matchweek-stop",
        type=int,
        default=DEFAULT_MATCHWEEK_STOP,
        help=f"Last NHL week number to include (default: {DEFAULT_MATCHWEEK_STOP}).",
    )
    parser.add_argument(
        "--matchweek-delay",
        type=float,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if output_path is None:
        ...
        output_path = Path("metadata") / f"nhl/{safe_season}.yaml"
    output_path = output_path.expanduser()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(yaml_text, encoding="utf-8")
    print(f"Wrote NHL metadata to {output_path}")
    return 0
