#!/usr/bin/env python3
"""Generate Moto2 metadata YAML using TheSportsDB rounds feed.

This script mirrors :mod:`generate_ufc_metadata` but targets Moto2
(``league_id 4436``). It groups SportsDB fixtures by round, emits a season
entry per round, and includes an episode for every scheduled Moto2 session.
"""
...
MOTO2_SESSIONS: List[Tuple[str, str]] = [
    ("Practice One", "practice-one"),
    ("Practice Two", "practice-two"),
    ("Qualifying One", "qualifying-one"),
    ("Qualifying Two", "qualifying-two"),
    ("Race", "race"),
]
...
def _normalise_event_name(value: Optional[str]) -> Optional[str]:
    if not value:
        return None
    cleaned = value.strip()
    if not cleaned:
        return None
    cleaned = re.sub(
        r"\s*grand prix moto2\b",
        " Grand Prix",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(r"\s*moto2\b", "", cleaned, flags=re.IGNORECASE)
...
def _season_summary(
    ...
    if match_count and date_span:
        return (
            ...
            "recorded for this Moto2 weekend."
        )
    if match_count:
        return (
    ...
    return (
        ...
        "Moto2 events available from TheSportsDB."
    )
...
def _session_summary(
    ...
    if event_date:
        ...
        summary_parts.append("Weekend focus date will be added once TheSportsDB updates.")
    if event:
        if event.get("strTimeLocal"):
            summary_parts.append(f"Listed local start: {event['strTimeLocal']}.")
        elif event.get("strTime"):
            summary_parts.append(f"Listed start: {event['strTime']} (UTC).")
    summary_parts.append(
        "Moto2 enforces Practice One/Practice Two/Qualifying One/Qualifying Two/Race "
        "slots even when TheSportsDB omits dedicated session entries so library "
...
def build_metadata(args: argparse.Namespace) -> dict:
    ...
    for round_number in sorted(events_by_round):
        ...
        event_name = _normalise_event_name(raw_event_name) or f"{args.round_label} {round_number}"
        ...
        fallback_event_date = dates[0] if dates else datetime.utcnow().date()
        event_slug = slugify(event_name)
        episodes = []
        for index, (session_title, session_slug) in enumerate(
            MOTO2_SESSIONS, start=1
        ):
            reference_event = (
                fixtures[index - 1] if index - 1 < len(fixtures) else artwork_event
            )
            summary_event = reference_event or artwork_event
            event_date = (
                _date_from_event(summary_event) if summary_event else fallback_event_date
            )
            event_date = event_date or fallback_event_date
            ...
            if args.fixture_poster_template:
                ...
                if download_assets:
                    ensure_asset_download(
                        _pick_episode_thumb(summary_event),
                        episode_poster_path,
                        context,
                        rate_limiter,
                        args.max_retries,
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Moto2 metadata YAML using TheSportsDB rounds feed.",
    )
    parser.add_argument(
        "--season",
        default="2025",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4436,
        help="TheSportsDB league/competition ID for Moto2.",
    )
    parser.add_argument(
        "--api-key",
        default="123",
    ...
    parser.add_argument(
        "--poster-url",
        default="posters/moto2/{season}/poster.jpg",
        help="Show-level poster path or URL (supports {season}).",
    )
    parser.add_argument(
        "--background-url",
        default="posters/moto2/{season}/background.jpg",
        help="Show-level background path or URL (supports {season}).",
    )
    parser.add_argument(
        "--summary",
        default=(
            "The {season} Moto2 World Championship spans every Grand Prix weekend "
    ...
    parser.add_argument(
        ...
        default="posters/moto2/{season}/s{matchweek}/poster.jpg",
        help=(
    ...
    parser.add_argument(
        ...
        default="posters/moto2/{season}/s{matchweek}/e{episode_index}.jpg",
        help=(
    ...
    parser.add_argument(
        ...
        "(defaults to metadata-files/moto2/{season}.yaml).",
    )
    parser.add_argument(
        "--insecure",
        action="store_true",
    ...
    parser.add_argument(
        "--round-label",
        default="Round",
        help="Label used when describing each Moto2 round (e.g. Round, Grand Prix).",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if output_path is None:
        ...
        output_path = Path("metadata-files") / f"moto2/{safe_season}.yaml"
    output_path = output_path.expanduser()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(yaml_text, encoding="utf-8")
    print(f"Wrote Moto2 metadata to {output_path}")
    return 0
